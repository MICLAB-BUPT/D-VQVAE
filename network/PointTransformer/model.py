...
try:
    ...
    flash_attn = None
    print("Flash Attention is not installed, please install it if you want to use it.")

from network.PointTransformer.serialization import encode
...
if __name__ == '__main__':
    print("start")
    feat = torch.randn(5*1024,6).to('cuda')
    grid_coord = torch.randn((1024*5,3)).to('cuda')
    coord = torch.randn((1024*5,3)).to('cuda')
    offset = torch.tensor([1024,2048,3072,4096,5120]).to('cuda')
    grid_size = torch.tensor([0.01]).to('cuda')
    datadict = {'feat':feat , 'offset':offset, 'coord':coord, 'grid_size':grid_size}
    model = PointTransformerV3().to('cuda')
    print("start")
    output= model(datadict)
    print("end")
    #print(output['feat']==datadict['feat'])
    print(output['feat'].shape)
    print(output['coord'].shape)
    print(output['coord']==datadict['coord'])
